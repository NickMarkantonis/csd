...
### Συγχρονισμός με Mutexes (Mutual Exclusion Locks)
#### Τι είναι ένα Mutex  
...
- Χρησιμοποιείται για να αποφευχθούν απρόβλεπτες τιμές όταν δύο ή περισσότερα threads προσπαθούν να γράψουν στην ίδια μεταβλητή ταυτόχρονα.  
- Όταν ένα thread θέλει να εκτελέσει μία κρίσιμη λειτουργία:  
  1. Κλειδώνει το mutex.  
  2. Εκτελεί την κρίσιμη λειτουργία.  
  3. Ξεκλειδώνει το mutex, επιτρέποντας στα υπόλοιπα threads να αποκτήσουν πρόσβαση.  
...
- Τα barriers (φράγες συγχρονισμού) είναι ένας μηχανισμός όπου όλα τα threads περιμένουν να φτάσουν σε ένα συγκεκριμμένο σημείο πρίν συνεχίσουν.
...
- Όταν φτάσει το 4ο thread, όλα συνεχίζουν ταυτόχρονα
...
1. Να τηρείται μία σταθερή σειρά κλειδώματος (π.χ πάντα πρώτα `lockA` και μετά `lockB`)
